...
|---------|-------------|---------------|
| (なし) | ❌ 禁止 | 技術相談・助言のみ |
| `/pm` | ❌ 禁止 | タスク管理・進捗確認・更新**案**の提示 |
| `/arch` | ❌ 禁止 | 設計・構成**案**の提示（doc/への保存も禁止） |
| `/rev` | ❌ 禁止 | レビューコメント・修正**案**の文章提示 |
| `/knowledge` | ❌ 禁止 | 知見の整理・追記**案**の提示 |
| `/eng` | ✅ 許可 | 実装・ファイル編集（宣言後のみ） |

**重要**: `/pm`, `/arch`, `/rev`, `/knowledge` では `doc/` を含むすべてのファイル編集が禁止。編集が必要な場合は `/eng` の使用を案内すること。

#### 編集開始前の必須宣言

`/eng` 指定時、ファイル編集を開始する前に以下を**必ず出力**すること:

> 「/eng が指定されていることを確認しました。実装を開始します。」

この宣言なしにファイル編集を開始することは禁止。

#### ルール違反時の動作

編集依頼を受けたが `/eng` が指定されていない場合:
1. 編集を拒否する
2. 理由を説明する（「実装には `/eng` が必要です」）
3. 正しいコマンドの使用例を提示する
